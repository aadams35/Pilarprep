param(
  ...
  [string]$AllowedOrigin = "https://pilarprep.app",
  [string]$SecondaryAllowedOrigin = "https://d2e0btay0ynyf.cloudfront.net",
...
$templatePath = Join-Path $repoRoot "backend\jobs_pipeline\template.yaml"
...
if (-not $SkipAgentCoreAuthorization) {
  ...
  try {
    ...
    & (Join-Path $PSScriptRoot "deploy-agentcore.ps1") `
      ...
      -Region $Region `
      -AllowedOrigin $SecondaryAllowedOrigin `
      -SecondaryAllowedOrigin $AllowedOrigin `
